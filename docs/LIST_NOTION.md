# Páginas Principais do Notion

- Arquitetura PWA to Play Store (B2C) com TWA
- Arquitetura de desenvolvimento de produtos MWT
- Método de Deploy na VM da GCP
- Configuração de Conexão: Prisma 5+
- 📘 O Guia Definitivo MW Technology: SDD & Arquitetura Multi-Agentes
- Guia de Instalação e Inicialização do GitHub Spec Kit (specify-cli)
- 🤖 Estratégia e Produto: Agente Autônomo de Vendas (Vitrine Interativa)
- Diretrizes Estritas de Segurança para Desenvolvimento de APIs (Node.js/TypeScript)
- 📘 PROJETO DE ARQUITETURA: ESTEIRA AUTO-REPLICÁVEL DE CODE REVIEW E ECOSSISTEMA DE AUTO-CURA (SELF-HEALING) VIA MULTI-AGENTES
- 🧬 DNA MW Technology | Manual de Cultura e Operação
- 📘 Manual de Arquitetura e Negócios: Micro-SaaS e Licenciamento Escalonável
