    ...
    <>
      <div className="flex gap-4 items-start relative w-full mb-6">
        ...
        <div 
          onClick={onCardClick}
          className={`
          flex-1 bg-white p-5 rounded-[32px] flex items-center justify-between shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300
        ...
        `}>
          <div className="flex gap-3 sm:gap-4 items-center">
            {photoUrl && (
              <div 
                className="w-14 h-14 sm:w-16 sm:h-16 rounded-[20px] overflow-hidden flex-shrink-0 cursor-pointer shadow-sm border border-gray-100 hover:opacity-90 transition-opacity"
            ...
            <div>
              <h3 className={`text-[19px] sm:text-xl font-bold flex items-center flex-wrap gap-2 ${isCompleted ? 'text-gray-400 line-through' : 'text-[var(--color-primary)]'}`}>
                {name}
                {dosage && (
                  <span className="text-[11px] sm:text-xs font-bold bg-[var(--color-secondary)]/20 text-[var(--color-primary)] px-2 py-1 rounded-full whitespace-nowrap line-through-none">
                    {dosage}
                  </span>
                )}
              </h3>
              ...
              {instructions && (
                <p className="text-gray-600 font-medium text-[14px] sm:text-[15px] mt-1 flex items-center gap-1.5 flex-wrap">
                  <span className="w-1.5 h-1.5 rounded-full bg-[var(--color-primary)]/50 flex-shrink-0"></span>
                  {instructions}
                  {frequency && <span className="text-[13px] sm:text-sm text-gray-400 ml-1 whitespace-nowrap">• {formatFrequency(frequency)}</span>}
                </p>
              )}
